...
Du kannst kitovu mit einer grafischen Oberfläche verwenden als auch über die Kommandozeile. Bevor du kitovu verwenden kannst, musst du jedoch die Konfigurationsdatei erstellen, siehe weiter unten.
    ...
    * ``kitovu validate`` prüft, ob deine gewählte Konfiguration fehlerlos ist.
...
``connections``: Die Verbindungen zu den Plattformen, von denen du deine Unterrichtsmaterialien synchronisierst, meistens ist das ein SMB-Skripteserver oder Moodle.

``name``: Ein von dir frei wählbarer Name für die jeweilige Plattform

``plugin``: Der Name der Plattform, der kitovu intern verwendet, damit eine Verbindung zustande kommt. Derzeit gibt es nur die fixe Bezeichnung ``smb`` oder ``moodle``.
...
``sources``: Die Plattformen, von denen du die Unterrichtsmaterialien synchronisierst. Gewisse Unterrichtsmodule sind sowohl auf Moodle als auch auf dem Skripteserver zu finden - hier kannst du für jede Plattform einen Eintrag wählen.
...
Bei der ersten Synchronsiation fragt dich kitvou nach allen benötigten Passwörtern. Für den Skripteserver ist dies im häufigsten Fall das Passwort, das du für alle anderen Dienste an der Schule verwendest.
...
Moodle hingegen ist ein Spezialfall. Um dich erfolgreich mit Moodle verbinden zu können, fragt dich kitovu bei der Erstverwendung nach dem Sicherheitsschlüssel. Den findest du folgendermassen:
    ...
    #. Logge dich via Browser in Moodle ein
    #. Navigiere zu den Einstellungen: klicke auf das Profilfoto oben rechts und wähle ``Einstellungen``, dann ``Sicherheitsschlüssel``.
    #. Kopiere den Eintrag unter "Schlüssel", der mit dem Servie "Moodle mobile web service" beschrieben wird. **Gib auf keinen Fall diesen Sicherheitsschlüssel weiter und behalte ihn für dich!**
    #. Füge den Sicherheitsschlüssel in kitovu ein.


Log: der FileCache
------------------

Kitovu hält in einer Datei fest, welche Dateien sich wie geändert haben, wenn du sie herunterlädst. Nach Ende eines Semesters bzw. nach Prüfungsende kannst du diese Datei wieder löschen - also in dem Fall, in dem keines der Unterrichtsmodule des vergangenen Semesters mehr synchronisiert werden sollte. Du findest die Datei in folgenden Verzeichnissen:
